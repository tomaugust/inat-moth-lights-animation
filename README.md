...
Planned enhancement notes: the UI may include an optional launch screen with a centered light switch, and an expandable side panel listing currently active known moths as cards. Cards should fade in and out as moths enter and leave, and hovering over a card should mirror the hover effect in the main animation. The hover target should be larger than the moth graphic so small, fast-moving moths are easier to select. A lightweight CSV-to-JSON conversion workflow is also planned for building the animation config from raw observation data.
