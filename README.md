...
- **UK-only for now.** The Worker is a single shared cache hard-set to one `PLACE_ID` — it doesn't read anything from the client's request, so per-visitor country (`src/geolocation.js`, built earlier but not wired into `app.js`) can't do anything useful until the Worker accepts and caches by a `place_id` the client sends. That's a separate follow-up, not done here.
  - **Known limitation to revisit when that follow-up happens:** the shared KV cache (see below) removes *redundant* refreshes of the same country's data, but it doesn't control *where* a refresh's outbound fetch to iNaturalist physically originates — that's still whichever Cloudflare colo happened to receive the visitor request that triggered the cache miss (Cloudflare routes by proximity to the visitor, unrelated to which country's data is being fetched). With only one country, that's a non-issue — there was only ever one "which colo refreshes this" race anyway. With N countries, each has its own independent refresh potentially originating from a different colo, which is still a real (if much smaller than pre-KV) exposure to hitting iNaturalist's rate limiter. The deterministic fix is a **Cloudflare Durable Object**: pin the upstream-fetching logic to one single, consistent worldwide location, so every outbound request to iNaturalist — for any country — always comes from the same place. **Deliberately not done now: Durable Objects require the paid Workers plan, and the project is staying on the free plan.** If this becomes a real problem later, Smart Placement (`placement = { mode = "smart" }` in `wrangler.toml`, free-plan compatible) is worth investigating first — it's a heuristic that biases a Worker's execution location toward wherever it mostly talks to, not a hard guarantee like a Durable Object, but costs nothing to try.
